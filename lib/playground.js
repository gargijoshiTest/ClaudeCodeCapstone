...
export function updateInfoPanel(color) {
  const nameEl = document.getElementById('pg-info-name');
  const hexEl  = document.getElementById('pg-info-hex');
  const rgbEl  = document.getElementById('pg-info-rgb');
  const hslEl  = document.getElementById('pg-info-hsl');
  if (nameEl) nameEl.textContent = color.name;               // textContent, never innerHTML
  if (hexEl)  hexEl.textContent  = color.hex;
  if (rgbEl)  rgbEl.textContent  = buildRgbString(color.hex);
  if (hslEl)  hslEl.textContent  = buildHslString(color.hex);
}

// ---- Contrast update ----

export function updateContrast(bgHex) {
  currentBgHex = bgHex;
  ...
  announce(
    `Selected ${selectedColor.name} ${selectedColor.hex}. Contrast ${ratio.toFixed(2)}:1 — ${pass ? 'WCAG AA Pass' : 'WCAG AA Fail'}`
  );
}
...
export async function initPlayground() {
  ...
  if (comboInput) {
    ...
    comboInput.addEventListener('blur', () => {
      // Delay allows mousedown on a listbox option to register before blur fires
